...
<head>
    ...
    <script>
        // Konfigurasi Tailwind tambahan jika belum tercakup di output.css
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
...
<aside class="h-screen w-72 fixed left-0 top-0 bottom-0 z-50 p-6 flex flex-col bg-surface-container-lowest border-r shadow-[20px_0_40px_rgba(0,0,0,0.02)]">
    ...
    <div class="flex items-center gap-4 px-4 py-6 mb-6 rounded-xl bg-surface-container-low">
        ...
        <div>
            <p class="font-body font-semibold text-on-surface text-sm leading-none">
    ...
    <nav class="flex-1 space-y-2">
        ...
        <a href="{{ route('admin.relawan') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg font-body font-medium text-sm text-gray-600 hover:bg-surface-container-low transition-all">
            <span class="material-symbols-outlined text-[20px]">group</span> Data Relawan
        </a>
        <a href="{{ route('admin.kegiatan') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg font-body font-medium text-sm text-gray-600 hover:bg-surface-container-low transition-all">
...
<main class="flex-1 ml-72 min-h-screen pb-20">
    ...
    <div class="p-8 space-y-10">
        ...
        @endif
        @if(session('pesan') == 'diarsipkan')
            <div class="p-4 mb-4 text-sm text-orange-800 rounded-lg bg-orange-50 border border-orange-200">
        ...
        <section class="space-y-4">
            <div class="flex justify-between items-end">
                ...
                <a href="{{ route('admin.kegiatan.tambah') }}" class="bg-primary text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-red-700 transition-all shadow-md shadow-red-100">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm font-body">
                        @forelse($kegiatan_aktif as $rk)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 font-bold">{{ $rk->nama_kegiatan }}</td>
                            <td class="px-6 py-4 text-gray-500">{{ $rk->lokasi }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('admin.kegiatan.edit', $rk->id_kegiatan) }}" class="text-blue-500 hover:bg-blue-50 p-1.5 rounded-lg" title="Edit">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </a>
                                    
                                    <form action="{{ route('admin.kegiatan.arsip', $rk->id_kegiatan) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" onclick="return confirm('Arsipkan kegiatan ini? (Akan hilang dari beranda)')" class="text-orange-500 hover:bg-orange-50 p-1.5 rounded-lg" title="Arsipkan">
                                            <span class="material-symbols-outlined text-lg">inventory_2</span>
                                        </button>
                                    </form>

                                    <form action="{{ route('admin.kegiatan.hapus', $rk->id_kegiatan) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Hapus permanen?')" class="text-red-400 hover:text-red-600 p-1.5 rounded-lg" title="Hapus">
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-400">Tidak ada kegiatan aktif saat ini.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse($antrian_pendaftar as $rp)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('admin.relawan.detail', $rp->id_pendaftaran) }}" class="text-blue-600 hover:bg-blue-50 p-1.5 rounded-full transition-all" title="Lihat Detail">
                                        <span class="material-symbols-outlined text-2xl">visibility</span>
                                    </a>

                                    <form action="{{ route('admin.relawan.terima', $rp->id_pendaftaran) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-green-600 hover:bg-green-50 p-1.5 rounded-full transition-all" title="Terima">
