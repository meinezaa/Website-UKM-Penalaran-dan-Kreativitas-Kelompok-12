@php
    ...
    // Kolom 'name' dan 'email' diambil dari tabel users bawaan Laravel
    $user_data = auth()->user();
    $email_otomatis = $user_data ? $user_data->email : '';
    $nama_otomatis  = $user_data ? $user_data->name : ''; 
@endphp
...
<body class="bg-pattern min-h-screen flex items-center justify-center p-6">
    <div class="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden">
        ...
        <form id="formRelawan" action="{{ route('pendaftaran.store') }}" method="POST" enctype="multipart/form-data" class="p-10">
            @csrf
