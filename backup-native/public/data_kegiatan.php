...
if (isset($_GET['hapus'])) {
    ...
    header("Location: data_kegiatan.php?pesan=terhapus");
    exit();
}

// QUERY AMBIL DATA (Tanpa ORDER BY spesifik dulu untuk menghindari fatal error kolom)
$query_kegiatan = mysqli_query($koneksi, "SELECT * FROM kegiatan");
...
<aside class="h-screen w-72 fixed left-0 top-0 bottom-0 z-50 p-6 flex flex-col bg-surface-container-lowest border-r shadow-[20px_0_40px_rgba(0,0,0,0.02)]">
    ...
    <div class="flex items-center gap-4 px-4 py-6 mb-6 rounded-xl bg-surface-container-low">
        ...
        <div>
            <p class="font-semibold text-gray-800 text-sm leading-none"><?= $_SESSION['nama_lengkap'] ?? 'Admin'; ?></p>
            <p class="text-[10px] text-gray-400 uppercase tracking-wider mt-1 font-bold">Super Admin</p>
        </div>
    </div>

    <nav class="flex-1 space-y-2">
        <a href="dashboard_admin.php" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-surface-container-low transition-all">
            <span class="material-symbols-outlined text-[20px]">dashboard</span> 
            <span class="text-sm font-medium">Dashboard</span>
        </a>
        <a href="data_relawan.php" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-surface-container-low transition-all">
            <span class="material-symbols-outlined text-[20px]">group</span> 
            <span class="text-sm font-medium">Data Relawan</span>
        </a>
        <a href="data_kegiatan.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary text-white shadow-md shadow-red-200">
    ...
    <div class="pt-6 border-t border-gray-100">
        <a href="logout.php" onclick="return confirm('Keluar?')" class="flex items-center gap-3 px-4 py-3 rounded-lg text-red-600 hover:bg-red-50 transition-all group">
...
<main class="flex-1 ml-72 min-h-screen">
    <header class="w-full sticky top-0 z-40 bg-white/80 backdrop-blur-md flex justify-between items-center px-8 py-4 border-b">
        ...
        <a href="tambah_kegiatan.php" class="bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-red-700 shadow-md shadow-red-100 transition-all">
    ...
    <div class="p-8 space-y-8">
        <section class="space-y-4">
            <div class="flex justify-between items-center px-2">
                ...
                <span class="text-[11px] font-black text-gray-400 uppercase tracking-widest">Total: <?= mysqli_num_rows($query_kegiatan); ?></span>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm font-body">
                        <?php if (mysqli_num_rows($query_kegiatan) > 0): ?>
                            <?php while($row = mysqli_fetch_assoc($query_kegiatan)): ?>
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="font-bold text-gray-800"><?= htmlspecialchars($row['nama_kegiatan'] ?? 'Tanpa Nama'); ?></p>
                                ...
                                <td class="px-8 py-5">
                                    <p class="font-semibold text-gray-700">
                                        <?= isset($row['tanggal_kegiatan']) ? date('d M Y', strtotime($row['tanggal_kegiatan'])) : (isset($row['tanggal']) ? date('d M Y', strtotime($row['tanggal'])) : 'Tanggal N/A'); ?>
                                    </p>
                                ...
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-1.5 text-gray-500">
                                        ...
                                        <span class="font-medium text-xs"><?= htmlspecialchars($row['lokasi'] ?? 'Lokasi N/A'); ?></span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-center">
                                    <div class="flex justify-center gap-1">
                                        <a href="edit_kegiatan.php?id=<?= $row['id_kegiatan']; ?>" class="w-8 h-8 flex items-center justify-center text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit">
                                            <span class="material-symbols-outlined text-xl">edit</span>
                                        </a>
                                        <a href="data_kegiatan.php?hapus=<?= $row['id_kegiatan']; ?>" onclick="return confirm('Hapus kegiatan ini?')" class="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Hapus">
