...
<body class="bg-surface font-body text-on-surface min-h-screen flex items-center justify-center p-4 md:p-8">
    ...
    <a href="login.php" class="fixed top-6 left-6 z-50 hidden md:flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-100 rounded-full text-xs font-bold text-gray-500 hover:text-primary transition-all shadow-sm group">
    ...
    <main class="w-full max-w-5xl relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-12 bg-white rounded-[2rem] md:rounded-[3rem] overflow-hidden custom-shadow border border-gray-50">
            ...
            <div class="hidden lg:flex lg:col-span-5 flex-col justify-end p-12 relative overflow-hidden bg-gray-400">
                <img src="foto/kegiatan3.jpg" class="absolute inset-0 w-full h-full object-cover opacity-60" alt="Kegiatan">
            ...
            <div class="lg:col-span-7 p-8 md:p-16 flex flex-col justify-center">
                ...
                <?php if ($pesan): ?>
                    <div class="mb-6 flex items-center gap-3 p-4 rounded-2xl text-xs font-bold border <?= $tipe_pesan == 'error' ? 'bg-red-50 text-red-700 border-red-100' : 'bg-green-50 text-green-700 border-green-100' ?>">
                        <span class="material-symbols-outlined text-lg"><?= $tipe_pesan == 'error' ? 'error' : 'check_circle' ?></span>
                        <?= $pesan; ?>
                    </div>
                <?php endif; ?>
                ...
                <form action="" method="POST" class="space-y-4">
                    ...
                    <button type="submit" name="register" class="w-full bg-primary hover:bg-red-700 text-white font-headline font-bold py-4 rounded-2xl shadow-lg shadow-red-100 hover:shadow-red-200 active:scale-[0.99] transition-all flex items-center justify-center gap-2 mt-4 text-xs uppercase tracking-[0.2em]">
                        DAFTAR SEKARANG
                ...
                <div class="mt-8 text-center">
                    <p class="text-xs text-on-surface-variant font-medium">
                        Sudah punya akun? <a href="login.php" class="text-primary font-bold hover:underline underline-offset-4">Log in</a>
                    </p>
                </div>
            </div>
        </div>
