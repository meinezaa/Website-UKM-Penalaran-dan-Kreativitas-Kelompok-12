...
<body class="bg-gray-100 p-6">
    ...
    <div class="max-w-4xl w-full mx-auto bg-white rounded-2xl shadow-xl overflow-hidden my-10">
        ...
        <form action="{{ route('admin.kegiatan.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8" novalidate>
            @csrf <div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="tanggal_pelaksanaan" value="{{ old('tanggal_pelaksanaan') }}" required class="w-full border border-gray-300 px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-red-500">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Jam Kegiatan</label>
                        <input type="text" name="jam_kegiatan" value="{{ old('jam_kegiatan') }}" required placeholder="08.00 - 12.00 WIB" class="w-full border border-gray-300 px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-red-500">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Batas Registrasi</label>
                        <input type="date" name="batas_registrasi" value="{{ old('batas_registrasi') }}" required class="w-full border border-gray-300 px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-red-500">
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Lokasi (Gedung/Sekolah)</label>
                    <input type="text" name="lokasi" value="{{ old('lokasi') }}" required placeholder="Contoh: SD Medokan Ayu 1" class="w-full border border-gray-300 px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-red-500">
                </div>
                <div>
            ...
            <div class="flex gap-4 pt-6 border-t border-gray-100">
                ...
                <a href="{{ route('admin.kegiatan.index') }}" class="flex-1 bg-gray-200 text-gray-700 font-bold py-4 rounded-xl text-center hover:bg-gray-300 transition-all">Batal</a>
            </div>
        </form>
    </div>
