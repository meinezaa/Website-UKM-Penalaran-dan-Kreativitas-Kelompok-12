...
// 3. Kelola Kegiatan (Proses Hapus)
...
// 4. Kelola Relawan (Tampil Data Lengkap + Hubungan ke Kegiatan + Search & Filter)
Route::get('/admin/kelola-relawan', function (Request $request) {
    $search = $request->input('search');
    $divisi = $request->input('divisi');

    // Menghubungkan pendaftaran_relawan ke users DAN ke tabel kegiatan
...
// 5. Kelola Relawan (Proses Hapus Berdasarkan ID Pendaftaran)
...
// 6. Kelola Relawan (Proses Impor Data dari CSV)
...
// 7. Detail Relawan (Disesuaikan bentuk select data kegiatan agar terbaca di file Blade terbaru)
Route::get('/admin/detail-relawan/{id_pendaftaran}', function ($id_pendaftaran) {
    ...
    // KOREKSI OPTIMAL: Membuat fake sub-object kegiatan agar struktur $relawan->kegiatan di Blade tidak patah/error
...
// 8. Proses Mengubah Status Seleksi Relawan (KOREKSI: Validasi mendukung Case-Insensitive)
Route::post('/admin/detail-relawan/{id_pendaftaran}/update-status', function (Request $request, $id_pendaftaran) {
    ...
    // Disimpan dengan format Huruf Kapital Semua agar konsisten di DB
...
// 9. Form edit kegiatan
// [A] ROUTE TAMPILKAN FORM EDIT (GET)
...
// [B] ROUTE PROSES UPDATE DATA (PUT - KOREKSI: Path folder simpan foto)
Route::put('/admin/edit-kegiatan/{id}', function (Request $request, $id) {
    ...
    if ($request->hasFile('foto_kegiatan')) {
        ...
        // KOREKSI: Disimpan ke sub-folder 'kegiatan' di disk public agar terbaca link asset()-nya
